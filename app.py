...
def get_unread_messages_count():
    """Count unread messages for the current user across all relevant categories"""
    try:
        user_id = session.get('user_id')
        is_admin = session.get('is_admin', False)
        
        if is_admin:
            # For admin, count platform inquiries and internal messages WHERE THEY ARE RECEIVER
            # AND only parent messages (to count threads, not every reply)
            ...
            # For portfolio users, count messages they received (portfolio OR internal)
            # AND only parent messages for initial contact, but for internal we want to count unread in their specific threads
            return Message.query.filter(
...
def get_latest_notifications():
    """Fetch latest unread notifications for polling including all categories"""
    try:
        user_id = session.get('user_id')
        is_admin = session.get('is_admin', False)
...
def track_visitor(username=None):
    ...
    if visitor_ip not in today_ips:
        ...
        # Visitor notification disabled per request
...
def dashboard_internal_messages():
    ...
    try:
        ...
        for msg in db_messages:
            ...
            other_party_name = msg.name
            other_party_email = msg.email
            
            # If current user is the sender, the other party is the receiver
            if msg.sender_id == user_id:
                receiver = User.query.get(msg.receiver_id)
                if receiver:
                    other_party_name = receiver.username
                    other_party_email = receiver.email
                elif msg.receiver_id == 'admin':
                    other_party_name = 'Administration'
                    other_party_email = 'admin@codexx.academy'

            all_messages.append({
                'id': str(msg.id),
                'display_name': other_party_name,
                'display_email': other_party_email,
                'message': msg.message,
                'date': msg.created_at.strftime('%Y-%m-%d %H:%M:%S'),
                'read': msg.is_read,
                'category': msg.category
...
def dashboard_internal_send():
    """Process sending a new internal message or continuing a thread"""
    user_id = str(session.get('user_id'))
    username = session.get('username')
    is_admin = session.get('is_admin')
    ...
    try:
        # Check if a thread already exists between these two parties
        # A thread is identified by the first message (parent_id is None)
        existing_thread = Message.query.filter(
        ...
        if existing_thread:
            # Continue the existing thread by adding a reply
            new_msg = Message(
                id=str(uuid.uuid4()),
                name='Admin' if is_admin else username,
                email='admin@codexx.academy' if is_admin else session.get('email', ''),
                ...
                sender_role='admin' if is_admin else 'member',
                receiver_id=receiver_id,
                parent_id=str(existing_thread.id),
                category='internal',
                created_at=datetime.utcnow()
            )
            db.session.add(new_msg)
            # Update parent thread timestamp to bring it to top
            existing_thread.created_at = datetime.utcnow()
            existing_thread.is_read = False # Mark for receiver
            db.session.commit()
            flash('Message added to thread.', 'success')
            return redirect(url_for('dashboard_internal_view', message_id=existing_thread.id))
        else:
            ...
            db.session.add(new_msg)
            db.session.commit()
            flash('New thread started.', 'success')
...
def dashboard_internal_view(message_id):
    """View an internal thread with administration"""
    is_admin = session.get('is_admin')
    user_id = str(session.get('user_id'))
    ...
    try:
        ...
        # Mark as read if user is the receiver or admin viewing platform/internal
        if message.receiver_id == user_id or (is_admin and message.category in ['platform', 'internal']):
            if not message.is_read:
                message.is_read = True
                db.session.commit()
            
        # Get replies (thread) - Support parent_id linking
        replies_objs = Message.query.filter_by(parent_id=str(message_id)).order_by(Message.created_at.asc()).all()
        
        all_replies = []
        for r in replies_objs:
            all_replies.append({
                'message': r.message,
        ...
        return render_template('dashboard/internal_view.html', message=message, replies=all_replies)
    except Exception as e:
